...
use BlackCat\Database\Packages\TenantDomains\Repository\TenantDomainRepositoryInterface;
...
class TenantDomainRepository implements TenantDomainRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\TenantDomains\Definitions::class);
    ...
    public function getByTenantIdAndDomainCi(int $tenantId, string $domainCi, bool $asDto = false): array|\BlackCat\Database\Packages\TenantDomains\Dto\TenantDomainDto|null {
        return $this->getByUnique([ 'tenant_id' => $tenantId, 'domain_ci' => $domainCi ], $asDto);
    }
    ...
    public function getIdByTenantIdAndDomainCi(int $tenantId, string $domainCi) {
        $row = $this->getByTenantIdAndDomainCi($tenantId, $domainCi, false);
