...
namespace BlackCat\Database\Packages\TenantDomains\Repository;
...
use BlackCat\Database\Packages\TenantDomains\Criteria;
use BlackCat\Database\Packages\TenantDomains\Dto\TenantDomainDto as Dto;
use BlackCat\Database\Packages\TenantDomains\Mapper\TenantDomainDtoMapper as RowMapper;
...
final class TenantDomainRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\TenantDomains\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\TenantDomains\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\TenantDomains\Dto\TenantDomainDto|null */
    public function getByTenantIdAndDomainCi(int $tenantId, string $domainCi, bool $asDto = false): array|\BlackCat\Database\Packages\TenantDomains\Dto\TenantDomainDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'domain_ci' => $domainCi ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\TenantDomains\Mapper\TenantDomainDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndDomainCi(int $tenantId, string $domainCi): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'domain_ci') . ' = :uniq_domain_ci';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_domain_ci' => $domainCi ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndDomainCi(int $tenantId, string $domainCi) {
        $row = $this->getByTenantIdAndDomainCi($tenantId, $domainCi);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\TenantDomains\Dto\TenantDomainDto|null */
    public function getByConstraint(mixed $constraint, bool $asDto = false): array|\BlackCat\Database\Packages\TenantDomains\Dto\TenantDomainDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\TenantDomains\Dto\TenantDomainDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\TenantDomains\Dto\TenantDomainDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\TenantDomains\Dto\TenantDomainDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\TenantDomains\Dto\TenantDomainDto|null {
