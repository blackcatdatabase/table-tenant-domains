...
namespace BlackCat\Database\Packages\TenantDomains\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\TenantDomains\Dto\TenantDomainDto;
use BlackCat\Database\Packages\TenantDomains\Mapper\TenantDomainDtoMapper;
 ...
 */
final class TenantDomainsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private TenantDomainRepository $tenantDomainRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'tenant_domains' => $this->tenantDomainRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\TenantDomains\ModuleException("Record {$idStr} not found for locking.");
            }
